...
const authService = {
  ...
  // Get user status
  getUserStatus: () =>
    apiClient.get('/users/status/').then(res => res.data),
  ...
  updateProfile: (profileData) => {
    ...
    return apiClient.patch('/users/profile/', formData, {
  ...
  changePassword: (oldPassword, newPassword) =>
    apiClient.post('/users/password/change/', {
  ...
  submitVerification: (verificationData) => {
    ...
    return apiClient.post('/users/verification/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    }).then(res => res.data);
  },
};
